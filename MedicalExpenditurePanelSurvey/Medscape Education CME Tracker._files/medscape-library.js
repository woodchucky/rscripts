/* Last Updated 5/1/2015 by ML */
/* Previously Updated 4/13/2015 by SC */
...
function adRollover()
{
	$("script[src*='pos%3'],iframe[src*='pos%3']").each(function() {
		...
		$(this).parent().bind('mouseover',function (e) {
			adRolloverPos = $(this).find(currTagName).filter(function() {return (this.src.match(/http:\/\/as\.medscape/) != null)})[0].src.match(/pos%3D\d+/)[0].substring(6);
		});
	});
...
function segVarParam(value) {
	...
	if ($('meta[name=metasegvar]').attr("content").match(segVarpar) != null) {var result = $('meta[name=metasegvar]').attr("content").match(segVarpar)[0].split("=")[1];}
	return result;
}
